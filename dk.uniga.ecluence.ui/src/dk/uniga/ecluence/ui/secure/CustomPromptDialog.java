 ...
 * Original work Copyright (c) 2011 GEBIT Solutions.
...
import dk.uniga.ecluence.ui.secure.CredentialItem.CharArrayType;
import dk.uniga.ecluence.ui.secure.CredentialItem.StringType;
import dk.uniga.ecluence.ui.secure.CredentialItem.YesNoType;

/**
 * Dialog for displaying authentication problems or requests for authentication
 * input from the user.
 *
 * The {@link CredentialItem CredentialItems} passed in the constructor are
 * updated to reflect the user-specified values as soon as the dialog is
 * confirmed.
 * 
 * Initial implementation by Carsten Pfeiffer(GEBIT Solutions)
 */
public class CustomPromptDialog extends TrayDialog {
	...
	/**
	 * Creates a dialog that presents the given CredentialItems and lets the user
	 * supply values for them.
	 * 
	 * @param shell
	 *            the shell for this dialog
	 * @param uri
	 *            the uri for which the information shall be displayed
	 * @param title
	 *            an optional title for this dialog
	 * @param items
	 *            the items to display and edit
	 */
	public CustomPromptDialog(Shell shell, URI uri, String title, CredentialItem... items) {
		super(shell);
		this.uri = uri;
		this.title = title;
	...
	protected Control createDialogArea(Composite parent) {
		...
		String tempInfoText = hasEditingItems() ? Messages.CustomPromptDialog_provideInformationFor
				: Messages.CustomPromptDialog_informationAbout;
		infoLabel.setText(MessageFormat.format(tempInfoText, uri.toString()));
	...
	/**
	 * Updates all {@link CredentialItem CredentialItems} with the user-supplied
	 * values.
	 */
